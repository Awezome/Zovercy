...
* 属  性：
		  ...
		  $link：连接标识
* 方  法：
		  connect($dbhost, $dbuser, $dbpw, $dbname = '', $pconnect = 0, $halt = TRUE, $dbcharset2 = '')//连接数据库 
		  select_db($dbname)//选择数据库 
		  fetch_array($query, $result_type = MYSQL_ASSOC)//将查询结果提取为数组 
		  fetch_first($sql)//以数组形势返回查询结果中第一条记录
		  result_first($sql)//返回查询结果中第一条记录的第一个单元  
		  query($sql, $type = '')//提交一条sql语句返回结果标志符
		  affected_rows() //  返回上次请求所影响的行号  
		  error()//返回MYSQL错误内容 
		  errno()// 返回MYSQL错误编号 
		  result($query, $row = 0)// 取出结果中第$row+1条记录（一个单元） 
		  num_rows($query)// 返回结果集的行数  
		  num_fields($query)//返回结果集的列数 
		  free_result($query)// 释放资源 
		  insert_id()// 取得最后插入的ID号
		  fetch_row($query)// 取出一行记录 
		  fetch_fields($query)// 从结果集中取得列信息并作为对象返回  
		  version()//返回MYSQL版本号 
		  close()// 关闭连接
		  selector//SELECT函数  
		  halt()//提示错误信息 
...
class MYSQL_DB{
	...
	* 参  数：$dbhost：数据库host名 
			  ...
			  $halt：设置是否显示错误信息
	*/ 
	function connect($dbhost, $dbuser, $dbpw, $dbname = '', $pconnect = 0, $dbcharset,$halt = TRUE) {
