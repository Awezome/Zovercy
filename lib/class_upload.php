...
class Upload{
	...
	function up_file($FILES){
		...
		if (!$this->FILES['name']) {$this->_error(1);return;}//检查文件名
		if (@is_dir($this->save_path) === false){$this->_error(2);return;}//检查目录
		if (@is_writable($this->save_path) === false) {$this->_error(3);return;}//检查目录写权限
		if (@is_uploaded_file($this->FILES['tmp_name']) === false){$this->_error(4);return;}//检查是否已上传
		if ($this->FILES['size'] > $this->max_size) {$this->_error(5);return;}//检查文件大小
		
		$this->_ext(); //获得文件扩展名
		if (!in_array($this->file_ext, $this->ext_arr[$this->upload_type]))  {$this->_error(6);return;}
		$this->_makedir();
		$this->_creat();
	}
	...
	private function _creat(){
		...
		if (move_uploaded_file($this->FILES['tmp_name'], $file_path) === false)   {$this->_error(7);return;}
	...
	private function _error($num){
		switch($num){
			case 0: $this->file_info="上传成功。";$this->up=1;break;
			case 1: $this->file_info="请选择文件。";break;
			case 2: $this->file_info="上传目录不存在。";break;
			case 3: $this->file_info="上传目录没有写权限。";break;
			case 4: $this->file_info="临时文件可能不是上传文件。";break;
			case 5: $this->file_info="上传文件大小超过限制。";break;
			case 6: $this->file_info="上传文件扩展名是不允许的扩展名。\n只允许" . implode(",", $this->ext_arr[$this->upload_type]) . "格式。";break;
			case 7: $this->file_info="上传文件失败。";break;
			default :break;
		}
	}
}
